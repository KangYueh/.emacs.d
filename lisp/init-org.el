...
;;   (suspend-frame)
;;   (call-process "gnome-screenshot" nil nil nil "-a" (concat
;;                                                      "\"" filename "\"" ))
;;   (insert (concat "[[" filename "]]"))
;;   (org-display-inline-images)
;;   )
(defun my/screenshot ()
  "Use gnome-screenshot to take a screenshot into a unique-named file and insert an org link."
  (interactive)
  (let* ((img-dir (concat (file-name-as-directory org-roam-directory) "img/"))
         (filename (concat img-dir (make-temp-name "screenshot_") ".png")))
    (unless (file-directory-p img-dir)
      (make-directory img-dir t))
    (suspend-frame)
    ;; gnome-screenshot with -a (area select) and -f (filename)
